...
CFG = {  # Дефолтные настройки
    ...
    # Через сколько записанных KB аудиоданных начать их воспроизведение из того же файла.
    # Если плеер догонит ТТС будет облом. Лучше не ставить это значение меньше 14.
    # Реальный профит заметен на длинных фразах у rhvoice-rest. Похоже гугл не стримит, без разницы.
    # Например: Очень длинная фраза заиграет через 3 секунды, а не через 40.
    # Яндекс не тестировал - ключ протух :)
    # Если равно 0 - воспроизведение начнется после полного сохранения файла
    'stream_race_tts': 0,
    'mpd': {
