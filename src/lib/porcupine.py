...
#
# see https://github.com/Picovoice/porcupine/blob/master/LICENSE
#
# original https://github.com/Picovoice/porcupine/blob/v1.8/binding/python/porcupine.py
...
class Porcupine(object):
    ...
    def __init__(
        ...
        new_api = tuple(int(x) for x in self.version.split('.')) > (1, 6, 0)

        init_func = library.pv_porcupine_init if new_api else library.pv_porcupine_multiple_keywords_init
        init_func.argtypes = [
            c_char_p,
            c_int,
            POINTER(c_char_p),
        ...
        self.process_func = library.pv_porcupine_process if new_api else library.pv_porcupine_multiple_keywords_process
